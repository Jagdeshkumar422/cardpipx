...
const DashPending = () => {
  ...
  useEffect(() => {
    const getPending = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${backend_URL}/transaction/pendingTransactions`);
  ...
  const depositTransactions = pendingData.filter((t) => t.TransactionType === "Deposit");
  const creditTransactions = pendingData.filter((t) => t.TransactionType === "Credit");
      ...
      {loading ? (
        ...
        <>
          ...
          {depositTransactions.length > 0 && (
            <div className="mb-8">
              <h2 className="text-lg font-semibold text-gray-700 mb-2">Pending Deposit Transactions</h2>
                  ...
                  {depositTransactions.map((data, index) => (
                    <tr key={index} className="hover:bg-gray-100 text-gray-700">
                      ...
                      <td className="p-3 border cursor-pointer">
                        {data.screenshot ? <img src={data.screenshot} draggable="false" className="w-8 h-8 object-cover" alt="Screenshot" onClick={() => handleImageModel(data.screenshot)} /> : "No Image"}
                      </td>
                      <td className="p-3 border text-yellow-600 font-semibold">{data.status}</td>
                      <td className="p-3 border flex items-center gap-5">
                        <FaCheckCircle className="text-xl text-blue-600 cursor-pointer" onClick={handleDepositApprove} /> <MdCancel className="text-2xl text-red-600 cursor-pointer" onClick={handleDepositReject} />
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {creditTransactions.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold text-gray-700 mb-2">Pending Credit Transactions</h2>
              <table className="w-full border border-gray-300">
                <thead>
