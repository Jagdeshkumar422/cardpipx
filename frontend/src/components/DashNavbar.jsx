        ...
        <div>
            <div className="pcDashNav md:flex items-center justify-between w-full px-[2rem] md:px-[7rem] h-[80px] hidden bg-[#f3f4f6]">
                <div className="flex items-center gap-8">
                    <Link to="/" className="w-full flex items-center gap-1 cursor-pointer"><img src={logo} alt="" className="w-[40] h-[40px]" /><h1 className="font-bold text-xl">CardpipX</h1></Link>
                    <Link to="/adminDash?tab=dash" className={`font-semibold text-sm  cursor-pointer ${location.pathname === "/dashboard" && "border-b-1 border-indigo-500 py-3 cursor-pointer"}`}>Dashboard</Link>
                </div>
            ...
            {profileModal && <div className="relative">
                <div className=" bg-[#f3f4f6] rounded-lg shadow-2xl w-[12rem] text-left h-[5rem] absolute top-2 right-[7%] flex flex-col items-start justify-center gap-1 text-sm">
                    <div className="w-full cursor-pointer hover:bg-white p-1 pl-4"><Link to="/profile">Profile</Link></div>
                    <button className="w-full cursor-pointer text-left hover:bg-white p-1 pl-4" onClick={handleLogOut}>Log Out</button>
                </div>
            </div>}
            <div className="mobileDashNav md:hidden">
                ...
                {isMobile && <div className="relative z-[500] bg-[#f3f4f6] w-[100%] h-[30vh]">
                    <div className="absolute z-[1000] top-0 flex flex-col justify-center items-start mt-4 w-full bg-[#f3f4f672] py-1">
                        <Link to="/adminDash?tab=dash" className={`w-full  font-semibold pl-4 py-2 text-sm  cursor-pointer ${location.pathname === "/dashboard" && "bg-indigo-50 text-indigo-600 border-l-4 border-indigo-500"}`}>Dashboard</Link>
                        ...
                        <div className="flex flex-col items-start justify-center pl-4 text-sm py-4">
                            ...
                            <p className="font-semibold text-gray-500">{currentUser.email}</p>
                        </div>
                        <div className="w-full pl-4 text-sm text-gray-500 font-semibold mb-4 cursor-pointer"><Link to="/profile">Profile</Link></div>
